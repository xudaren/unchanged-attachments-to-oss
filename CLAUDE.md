...
- 配置阿里云 OSS 的 `region`、`bucketName`、AK/SK、可选标准 `endpoint`、可选自定义访问域名、Object Key 前缀、签名有效期和 Bucket 级公共读开关；AK/SK 必须经用户主密码加密后随 Vault 同步，保存时校验凭证连通性，失败则报错并阻止保存。
- 感知图片、视频、音频、PDF 的上传，先建立可恢复本地副本，再上传 OSS、验证引用替换，最后按安全条件清理本地文件。
- 按公共读开关以公共 URL 或动态签名渲染已上传文件的访问路径。
...
- 成本与风险：插件无服务端，部署简单；AK/SK 以主密码派生密钥加密后存入 Vault，主密码和派生密钥不持久化，用户需在每次插件加载后解锁。用户承担 OSS 存储/流量成本，可选核验还需要 ListObjects 权限。客户端凭证模式只适合个人可信设备，不应分发共享高权限 AK。公共读开启后引用为未签名公共 URL，可随 Vault 直发 git 或在线文档平台；上传、删除、核验等写操作仍需 AK/SK。阿里云对默认域名 `{bucket}.{endpoint}` 的浏览器访问按地域和 Bucket 创建时间强制下载图片等类型，可选自定义域名作为统一的浏览器访问域名恢复直开预览。
...
- [配置](docs/configuration.md)：字段校验、V4 凭证探针、Endpoint 限制、自定义访问域名与存储身份。
...
- md 中新上传引用必须以标准 markdown 语法存储未签名公共 URL `![](https://{访问域名}/{percentEncodedKey})`，访问域名为已配置的自定义域名，未配置时为 `{bucket}.{endpoint}`；遗留 `oss:///{percentEncodedKey}`（三斜杠表示无 host 的绝对路径）与 host 为默认域名 `{bucket}.{endpoint}` 的公共 URL 引用必须永远被上传、渲染、删除、核验识别，但 `oss://` 形态禁止新写入。Object Key 的每个路径段必须 percent-encode。禁止使用 raw 空格/括号等不安全目标、wikilink、HTML 内嵌或带签名 URL。上传、渲染、删除、核验必须共用唯一 `OssReferenceCodec`，禁止各模块复制简单正则。
...
- 每次代码实现完成并通过验证后，默认执行 `npm run deploy:test` 部署到 `/Users/xukai/xukai_workspace/knowledge-base-test/.obsidian/plugins/unchanged-attachments-to-oss`，方便用户立即验证；部署只更新 `main.js`、`manifest.json`、`styles.css`，禁止覆盖测试 Vault 中的 `data.json`。
