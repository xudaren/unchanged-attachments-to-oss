...
export function mountMediaLabel(media: HTMLElement, name: string, key: string, host?: Element): HTMLElement | null {
  ...
  if (media.tagName === "AUDIO") container?.classList?.add("oss-audio-caption-host");
  for (const label of Array.from(container?.children ?? [])) {
    if (label.classList.contains("oss-media-label") && (label as HTMLElement).dataset.ossKey !== key) {
      label.remove();
    }
  }
  const existing = container?.querySelector?.<HTMLElement>(`:scope > .oss-media-label[data-oss-key="${cssEscape(key, media.ownerDocument)}"]`);
  if (existing) {
...
function cssEscape(value: string, document: Document): string {
  const css = (document.defaultView as (Window & { CSS?: typeof CSS }) | null)?.CSS;
